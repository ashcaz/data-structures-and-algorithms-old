...
function getCurrentEvents(request,response){
  const frontEnd = mapCurrentEvents();
  // console.log(mapCurrentEvents());
  response.status(200).json(frontEnd);
}

const mapCurrentEvents = () => {
  const theNews = currentEvents.news.map(obj => new Event(obj));
};
...
const countNumberOfElements = (arr) => {
  let num = arr.reduce((accumulator, value, idx) => {
    accumulator++;
    return accumulator;
  },0);
  return num;
...
const returnNames = (arr) => {
  let characters = arr.reduce( (char, val, idx) => {
    char.push(val.name);
    return char;
  }, []);
  return characters;
...
const reversedString = (str) => {
  ...
  let reversed = spiltArr.reduce( (newString, currentLetter) => {
...
const countNumberOfChildren = (arr) => {
  let kids =arr.reduce( (allKids,fam) =>{
    let famTotal = fam.children.reduce( (num, val, idx) => {
      num = idx + 1;
      return num;
    }, 0);
    console.log(famTotal);
    allKids = allKids + famTotal;
    console.log(allKids);
    return allKids;
  }, 0);
  return kids;
